...
Агент `feedback-agent` сохраняет результаты в Redis и **восстанавливает состояние при перезапуске**:
...
При старте агент читает все ключи `hr:candidate:*` и счётчики из Redis.

### 5. Динамическое масштабирование

Реализован автоматический скейлер `scripts/auto_scaler.go`, который:
- Мониторит длину очереди через NATS `/varz` endpoint
- Автоматически запускает новые экземпляры агентов при превышении порога (по умолчанию 10)
- Ограничивает максимальное количество реплик (по умолчанию 5)
- NATS queue groups распределяют задачи между репликами

### 6. Аукционное распределение задач

Оркестратор реализует аукционный механизм:
- Публикует запрос `hr.auction.bid_request` с ID задачи
- Go-агенты подписаны на SubjectAuction и отвечают bid с `cost`, `skill`, `availability`
- Оркестратор выбирает лучшего исполнителя по минимальной стоимости и максимальной доступности
- Выбранный агент получает задачу через свой Subject
